# From https://github.com/neo/middleman-gh-pages
...
file GH_PAGES_REF => BUILD_DIR do
  ...
  cd BUILD_DIR do
    ...
    if `git branch -r` =~ /#{BRANCH}/
      sh "git checkout #{BRANCH}"
    else
      sh "git checkout --orphan #{BRANCH}"
      sh "touch index.html"
      sh "git add ."
      sh "git commit -m 'initial #{BRANCH} commit'"
...
# Fetch upstream changes on #{BRANCH} branch
task :sync do
  cd BUILD_DIR do
    sh "git fetch #{remote_name}"
    sh "git reset --hard #{remote_name}/#{BRANCH}"
  end
end
